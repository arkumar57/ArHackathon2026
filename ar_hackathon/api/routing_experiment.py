...
Strategy:
1. Congestion-aware weighted shortest path (Dijkstra). Each call rebuilds the
   current congestion snapshot, so edges/nodes at capacity carry an estimated
   "time until a slot frees" penalty. Units detour around narrow aisles when a
   detour beats waiting, and fall back to the direct route (waiting one step)
   when nothing better exists.
2. Pod assignment. An idle empty unit claims its nearest waiting pod, with
   ties broken by entry time and destination-station load. Claims are tracked
   module-wide so different units do not chase the same pod. Picking up every
   waiting pod at a node is automatic, which gives natural batching for
   multi-capacity units.
3. Positioning. An idle unit with no waiting pods heads back toward the
   nearest storage so it is already in place when the next pod appears.
4. Everything static (adjacency, base weights, node/edge capacities, pairwise
   distances) is computed once and cached; each call does a single Dijkstra
   plus table lookups.
